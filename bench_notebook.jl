...
# ╔═╡ 5ca41fa1-4ef3-4340-9ad1-8b7e784a4ef3
begin
using CairoMakie
...
# ╔═╡ 3ec91789-b20a-4a00-a33b-3944b4400a16
begin
using LinearAlgebra
...
# ╔═╡ 25c9ee23-4904-464c-a72a-166014af5eaf
begin
...
# ╔═╡ 9074fcc0-1a47-4936-b4c2-3c4fdc1c7e4a
md"""Lets check that answers, including the partials of the dual numbers, match."""

# ╔═╡ 5a0346e3-1c43-4956-8dda-7f76be76de43
begin
...
# ╔═╡ 977b00d8-2a13-4a5d-adfd-053e3ad6da0a
cmpplot(["Clang", "ExponentialUtilities.jl"], [1.0, t_exputils/t_clang])

# ╔═╡ f17a6025-0738-4a29-a08e-3130987492b7
md"""Okay, so our initial C++ implementation is 4x faster. Lets try a Julia translation:"""

# ╔═╡ 133ce9aa-0b6f-4670-9ab6-3365853155a4
begin
    resexpm = @time @eval do_multithreaded_work!(expm!, Bs, As, testrange)
	@assert approxd(res, resexpm)
end

# ╔═╡ f331da59-58f8-4b9d-b829-284c0912015c
...
# ╔═╡ a8ef3f04-bc76-443e-aba2-c4c859af7e38
md"""Okay -- so no improvement yet.
[Julia PR#52038](https://github.com/JuliaLang/julia/pull/52038) promises to improve the compiletime and runtime of `mul!` for Julia 1.11, while I'm still on the latest release (1.9.3). So, lets try implementing that.
"""

# ╔═╡ 0877841c-2bfc-4a5c-819a-c407d981177c
begin
...
# ╔═╡ e96a07bd-5cf1-4eaf-8456-ceef830c55d8
begin
    resexpmcm = @time @eval do_multithreaded_work!(expm_custommul!, Bs, As, testrange)
	@assert approxd(res, resexpmcm)
end

# ╔═╡ 0d5784b0-0940-4d2f-bc23-e22d0ca1a204
t_expm_custommul = @elapsed do_multithreaded_work!(expm_custommul!, Bs, As, testrange)

# ╔═╡ 388bb73c-28bb-4c4d-a28a-89977c3a1d47
cmpplot(
	["Clang", "ExponentialUtilities.jl", "expm!", "expm_custommul!"],
	[t_clang, t_exputils, t_expm, t_expm_custommul] ./ t_clang
)

# ╔═╡ 36b332d8-3257-4961-9782-e85400ff13fe
md"""That is much better! We are now most of the way there. `@time`ing a compiled run, we see that GC time is fairly substantial:"""

# ╔═╡ 8fb8e02e-96dd-44e7-a94a-186edbaf7b25
@time do_multithreaded_work!(expm_custommul!, Bs, As, testrange)

# ╔═╡ 4ab2dcee-9ca9-458e-b72f-732f16d11fbd
md"""The C++ of course is also heap-allocating, but using SBRM (scope-based resource management) avoids the GC-overhead, particularly in multithreaded contexts. Julia does not support SBRM -- the language doesn't provide the hooks (e.g. destructor calls at end of scope, copy constructors when passing or returning by value -- nor does it have a by value vs reference concept).

Suffice it to say, we must look for alternatives. An altenative that has been suggested was that the compiler may eventually be able to store non-escaping objects in a task-local storage (while allowing them to be collected, like in a weakkey dict), so that we can reuse them. Lets try to do something like this manually."""

# ╔═╡ 2d615c08-0195-4f63-9552-e13956aa02e2
begin
...
# ╔═╡ f9ee9758-083c-499b-ac21-0721cfec9caf
begin
    restls = @time @eval do_multithreaded_work!(expm_tls!, Bs, As, testrange)
	@assert approxd(res, restls)
end

# ╔═╡ 4c8c4663-bbfd-41ce-aedc-4e24814090bd
t_tls = @elapsed do_multithreaded_work!(expm_tls!, Bs, As, testrange)

# ╔═╡ 96eea595-c46f-4342-8d32-e722e4bf78cd
cmpplot(
	["Clang", "ExponentialUtilities.jl", "expm!", "expm_custommul!", "expm_tls!"],[t_clang, t_exputils, t_expm, t_expm_custommul, t_tls] ./ t_clang
)

# ╔═╡ 1a4cee69-3458-48ee-8a0f-6a787c1c623a
@time do_multithreaded_work!(expm_tls!, Bs, As, testrange)

# ╔═╡ 6fa30670-d594-4d4e-b430-144e1f15858e
@benchmark expm_tls!(copyto!($B2, $A2))

# ╔═╡ 05a76b28-9305-49c1-b4b5-07082ea044a1
@benchmark clangexpm!(copyto!($B2, $A2))

# ╔═╡ c53fb8f3-d912-45f8-a30a-82c25c14b802
begin
B5 = Bs[end][end][end]
A5 = As[end][end][end]
size(A5), eltype(A5)
...
# ╔═╡ 19c6c03e-5232-4a9a-aff9-7e2891edef30
@benchmark expm_tls!(copyto!($B5, $A5))

# ╔═╡ e90a878a-a30c-4845-98a5-7f98f72a044d
@benchmark clangexpm!(copyto!($B5, $A5))

# ╔═╡ 468e2347-32b1-4b7e-a73f-a1999ca2ac76


# ╔═╡ 616d5995-d643-469b-aa06-f7a4d5ae96a9


# ╔═╡ 9afc162a-34c2-40bf-9e08-e942e3fc48df
...
julia_version = "1.9.4"
manifest_format = "2.0"
project_hash = "6963cdd34229c6205968a662f17313f0725c4e77"
